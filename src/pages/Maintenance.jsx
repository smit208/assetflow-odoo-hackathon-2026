  ...
  async function load() {
    const [{ data: reqs }, { data: profs }] = await Promise.all([
      supabase.from('maintenance_requests')
        .select('id, asset_id, raised_by, status, priority, issue, technician_name, photo_url, condition_notes, created_at, assets(id, tag, name)')
    ...
    setProfileMap(pMap)
    setRequests((reqs || []).map(r => ({ ...r, profiles: pMap[r.raised_by] || null })))
  }
